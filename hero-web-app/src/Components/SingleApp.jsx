...
const SingleApp = ({ app }) => {
  ...
  return (
    <Link to={`/appdetails/${app.id}`}>
      <div className="appCard p-2 bg-white rounded-md shadow-sm">
        ...
        <div className="flex justify-between items-center py-1">
          ...
          <span className="bg-purple-100 px-2 flex items-center gap-2 rounded-sm">
            <span>
              <FaStar className="text-purple-400" />
            </span>
            <span className="text-purple-400">{ratingAvg}</span>
          </span>
        </div>
      </div>
    </Link>
